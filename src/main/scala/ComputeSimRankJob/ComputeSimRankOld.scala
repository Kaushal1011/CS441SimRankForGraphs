...
object ComputeSimRankJobOld {
  ...
  private class MyCustomWritableOld extends Writable {
  ...
  private class JaccardMapperOld extends Mapper[LongWritable, Text, Text, MyCustomWritableOld] {

    private val logger = CreateLogger(classOf[JaccardMapperOld])

    override def map(key: LongWritable, value: Text, context: Mapper[LongWritable, Text, Text, MyCustomWritableOld]#Context): Unit = {
      ...
      val customWritable = new MyCustomWritableOld()
  ...
  private class JaccardReducerOld extends Reducer[Text, MyCustomWritableOld, Text, MyCustomWritableOld] {

    private val logger = CreateLogger(classOf[JaccardReducerOld])

    override def reduce(key: Text, values: java.lang.Iterable[MyCustomWritableOld], context: Reducer[Text, MyCustomWritableOld, Text, MyCustomWritableOld]#Context): Unit = {
...
//      val sb = node2s._1.toString + "|" + node2s._2.toString
  ...
  def main(args: Array[String]): Unit = {
    ...
    job.setJarByClass(classOf[JaccardMapperOld])


    job.setMapperClass(classOf[JaccardMapperOld])
    job.setCombinerClass(classOf[JaccardReducerOld])
    job.setReducerClass(classOf[JaccardReducerOld])

    // Set the custom writable class as the output value class for the Mapper
    job.setMapOutputValueClass(classOf[MyCustomWritableOld])
